...
ADAPTER_DIR = os.path.join(RESULT_DIR, "lora_adapter")
...
if __name__ == "__main__":
    ...
    simularity = t_eval.calc_simularity(text_feats, pic_feats, os.path.join(RESULT_DIR, "lora_"+SPLIT+"_preds.csv"))
    ...
    with open(os.path.join(RESULT_DIR, f"lora_{SPLIT}_summary.json"), "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)
    print(f"\n预测明细已保存: results/lora_{SPLIT}_preds.csv")
    print(f"结果摘要已保存: results/lora_{SPLIT}_summary.json")
